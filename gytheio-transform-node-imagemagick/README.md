...
The `ImageMagickontentTransformerNodeWorker` from the `gytheio-transform-worker-imagemagick` dependency is used
...
AMQP broker host, request queue, and reply queue, i.e.:
    ...
    java -jar target/gytheio-transform-node-imagemagick-0.1-SNAPSHOT-jar-with-dependencies.jar localhost alfresco.transform.worker.request.image alfresco.transform.worker.reply.image
