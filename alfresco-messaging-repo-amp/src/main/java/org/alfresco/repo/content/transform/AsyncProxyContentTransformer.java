 ...
 * @author dward
 */
public class AsyncProxyContentTransformer extends AbstractAsyncContentTransformer
{
    private AsyncContentTransformerWorker worker;

    public AsyncProxyContentTransformer()
    {
    }
     ...
     */
    public void setWorker(AsyncContentTransformerWorker worker)
    {
        this.worker = worker;
    }
     ...
     */
    public AsyncContentTransformerWorker getWorker()
    {
        return this.worker;
    }
    ...
    {
        return this.worker.getProgress(runningTransformationId);
    }

    @Override
    protected String transformInternal(ContentReader reader, ContentWriter writer, TransformationOptions options,
            boolean executeAsynchronously) throws Exception
    {
        TransformationOptionLimits original = options.getLimits();
        try
        {
            ...
            // Perform the transformation
            return this.worker.transform(reader, writer, options, executeAsynchronously);
        }
        finally
        {
            options.setLimits(original);
